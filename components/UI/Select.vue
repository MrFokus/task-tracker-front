...
watch(() => props.listSelect, () => {
  if (props.listSelect.length > 0 && !model.value) {
    select(0)
  }
}, { immediate: true })
...
<template>
  <div v-out-click="() => { isActive = false }" :class="['select', isActive ? 'active' : '']">
    <button type="button" @click="isActive = !isActive" :style="{ justifyContent: model ? '' : 'flex-end' }" class="open white">
      {{ model }}
    ...
    <ul v-if="isActive" class="list-select-component">
      <li v-for="(option, index) in listSelect">
        <button type="button" @click="select(index)" :class="[option === model ? 'selected' : '']">
          {{ option }}
