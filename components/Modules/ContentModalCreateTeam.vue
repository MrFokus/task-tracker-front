...
function addPeople(user: User) {
    ...
    if (listPeople.value.length<0 || !listPeople.value.find((el) => el.id === user.id)) {
        listPeople.value.push({ ...user, role: listSelectRole.value ? listSelectRole.value[listSelectRole.value?.length - 1].id : listPeople.value.length ? 3 : 1 })
    ...
    <form @submit.prevent="createTeam" class="white-window column">
        ...
        <div class="content-modal modal-block column">
            ...
            <div class="input-container column">
                ...
                <Search :attributes="{placeholder:'@login'}" v-model:is-search="isUserSearch" v-model="searchUser" @select="addPeople">
                    <li @click="addPeople(user)" class="profile-search" v-for="user in listSearchUser">
                ...
                <ul class="includes-participants column">
                    <li v-if="listPeople" v-for="(user, index) in listPeople" class="include-profile">
                        ...
                        <button @click="deletePeople(index)" class="white delete-user">
